...
static int janus_rtcp_has_nacks(char *packet, int len);
...
static void janus_streaming_rtcp_remb_send(janus_streaming_rtp_source *source) {
	...
	/* Send the packet */
	int sent = 0;
//JANUS_LOG(LOG_HUGE, ">>> BITRATE send to %08x %04x\n", ((struct sockaddr_in*)&source->video_rtcp_addr)->sin_addr.s_addr, ((struct sockaddr_in*)&source->video_rtcp_addr)->sin_port);
	if((sent = sendto(source->video_rtcp_fd, rtcp_buf, rtcp_len, 0,
			&source->video_rtcp_addr, sizeof(source->video_rtcp_addr))) < 0) {
		JANUS_LOG(LOG_ERR, "Error in sendto... %d (%s)\n", errno, strerror(errno));
	} else {
...
static void janus_streaming_rtcp_nacks_send(janus_streaming_rtp_source *source, char *buf, int len) {
	if(source == NULL || source->video_rtcp_fd < 0 || source->video_rtcp_addr.sa_family == 0)
		return;
	/* Filter NACKS portion of compound packet */
	char rtcp_buf[16];
	int rtcp_len = 0;
		...
		while(cl != NULL) {
			...
			if(!strcasecmp(type->value, "rtp")) {
				...
				if((mp = janus_streaming_create_rtp_source(
						...
						/* Uso la porta RTP+1 per RTCP, anche se poi una è socket unix
						   e l'altra socket UDP/IP */
						//(vrtcpport && vrtcpport->value) ? atoi(vrtcpport->value) : 0,
						(vport && vport->value) ? atoi(vport->value)+1 : 0,
...
void janus_streaming_incoming_rtcp(janus_plugin_session *handle, int video, char *buf, int len) {
	...
	if(!video && (source->audio_rtcp_fd > -1) && (source->audio_rtcp_addr.sa_family != 0)) {
		...
		if(bw > 0) {
			...
			if(!source->lowest_bitrate || (bw > source->lowest_bitrate))
				source->lowest_bitrate = bw;
		}
		if(janus_rtcp_has_nacks(buf, len)) {
			/* We got a NACKs, pass it along */
			JANUS_LOG(LOG_HUGE, "  -- NACKs received\n");
			janus_streaming_rtcp_nacks_send(source, buf, len);
		}
	}
}

void janus_streaming_slow_link(janus_plugin_session *handle, int uplink, int video, int nacks) {
	JANUS_LOG(LOG_WARN, "[%p] Slow link up:%d video:%d naks:%d\n", handle, uplink, video, nacks);
}
...
static int janus_streaming_create_unix_fd(int port, in_addr_t mcast, const janus_network_address *iface, const char *listenername, const char *medianame, const char *mountpointname) {
	char socket_name[64];
...
janus_streaming_mountpoint *janus_streaming_create_rtp_source(
	...
	if(doaudio) {
		...
		audio_fd = janus_streaming_create_unix_fd(aport, amcast ? inet_addr(amcast) : INADDR_ANY, aiface,
		...
		if(artcpport > 0) {
			...
			audio_rtcp_fd = janus_streaming_create_unix_fd(artcpport, amcast ? inet_addr(amcast) : INADDR_ANY, aiface,
				"Audio", "audio", name ? name : tempname);
			if(audio_rtcp_fd < 0) {
	...
	if(dovideo) {
		...
		video_fd[0] = janus_streaming_create_unix_fd(vport, vmcast ? inet_addr(vmcast) : INADDR_ANY, viface,
		...
		if(simulcast) {
			if(vport2 > 0) {
				...
				video_fd[1] = janus_streaming_create_unix_fd(vport2, vmcast ? inet_addr(vmcast) : INADDR_ANY, viface,
					"Video", "video", name ? name : tempname);
				if(video_fd[1] < 0) {
			...
			if(vport3 > 0) {
				...
				video_fd[2] = janus_streaming_create_unix_fd(vport3, vmcast ? inet_addr(vmcast) : INADDR_ANY, viface,
					"Video", "video", name ? name : tempname);
				if(video_fd[2] < 0) {
...
#if 0
	if(vrtcpport > 0) {
		/* Connetto subito il socket all'IP locale */
		((struct sockaddr_in*)&(live_rtp_source->video_rtcp_addr))->sin_family = AF_INET;
		((struct sockaddr_in*)&(live_rtp_source->video_rtcp_addr))->sin_port = htons(vrtcpport);
		((struct sockaddr_in*)&(live_rtp_source->video_rtcp_addr))->sin_addr.s_addr = htonl(INADDR_LOOPBACK);
	}
#endif
		...
		for(i=0; i<num; i++) {
			if(fds[i].revents & (POLLERR | POLLHUP)) {
				...
				if(pipe_fd != -1 && fds[i].fd == pipe_fd) {
					...
					/* La gestione socket UNIX di videomux inoltra sia RTP che RTCP sullo stesso socket,
					   quindi anziché differenziare a monte (videomux) occorre differenziare a valle (janus). */
					if((((unsigned char)buffer[1] >= 192) && ((unsigned char)buffer[1] <= 195)) ||
					   (((unsigned char)buffer[1] >= 200) && ((unsigned char)buffer[1] <= 210))) {
						/* RTCP packet, skip */
						continue;
					}
...
{
  unsigned short seq = ntohs(packet.data->seq_number);
  if(seq != ((debug_seq+1)&0xffff))
  {
    JANUS_LOG(LOG_WARN, "[%s] Lost seq, atteso %d arrivato %d\n", name, (debug_seq+1)&0xffff, seq);
  }
  debug_seq = seq;
}
					packet.seq_number = ntohs(packet.data->seq_number);
