...
#define JANUS_STREAMING_VERSION_STRING	"0.0.8 (Saet)"
...
static int janus_streaming_create_unix_fd(int port, in_addr_t mcast, const janus_network_address *iface, char *host, size_t hostlen,
		const char *listenername, const char *medianame, const char *mountpointname, gboolean quiet) {
...
static void janus_streaming_rtp_source_free(janus_streaming_rtp_source *source) {
	...
	if(source->keyframe.temp_keyframe != NULL)
		g_list_free_full(source->keyframe.temp_keyframe, (GDestroyNotify)janus_streaming_rtp_relay_packet_free);
